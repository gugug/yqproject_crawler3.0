...
def get_tpw(tpdict):
    ...
    for (keys, values) in tpdict.items():
        print "博文id", keys, "博文", values
        topic_patternts = re.compile('【(.*?)】')
        topic = topic_patternts.findall(values)
        if len(topic) > 0 and topic not in title_list:
            ...
            word = Keyword().combine_keywords(values)
    ...
    return bid_list, title_list, tw_list
