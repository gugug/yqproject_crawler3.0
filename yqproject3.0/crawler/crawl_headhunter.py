...
class Sina():
    ...
    def save_txt(self):
        ...
        try:
            ...
            data_file = open(self.data_dir + 'uid=' + self.get_uid() + '.txt', 'w+')
            data_file.write(
                '用户名: ' + self.get_name()
                + '\nid: ' + self.get_uid()
                + '\n微博数: ' + self.get_weibo_num()
                + '\n关注数: ' + self.get_follows_num()
                + '\n粉丝数: ' + self.get_fans_num()
                + '\n是否大v： ' + self.isbig_v()
                + '\n性别： ' + self.get_sex()
                + '\n地区： ' + self.get_area()
                + '\n生日： ' + self.get_birthday()
                + '\n认证： ' + self.get_authentication()
                + '\n认证信息： ' + self.get_authentic_info()
                + '\n简介： ' + self.get_brief_intro()
                + '\n用户标签: ' + self.get_tags()
                + '\n'
            )
            data_file.close()
