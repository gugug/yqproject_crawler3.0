...
from crawler.MoblieWeibo import MoblieWeibo
...
class WeiboPage:
    ...
    def get_page(self, p1, p2, s):
        ...
        try:
            ...
            print len(one.items())
            if len(one.items()) != 0:
                self.write_hunter_txt(one.keys(), one.values())  # 检测事件
                # self.get_comment(one.keys(), one.values())
                # self.get_forward_path(one.keys(), one.values())

                all_topic = self.get_topic(one.keys(), one.values(), s)  # 搜索事件，三个列表元组 ,(话题, id, 博文)
                ...
                print "字典未增加检测事件的数量", len(WeiboPage.searched_tpdict)

                WeiboPage.searched_tpdict.setdefault(one.keys()[0], tp_and_wk)  # 用字典保存，这个猎头检测到的新事件,键是猎头，值是元组列表
                print WeiboPage.searched_tpdict

                print "字典增加检测事件后的数量", len(WeiboPage.searched_tpdict)
                print WeiboPage.searched_tpdict.keys(), WeiboPage.searched_tpdict.values()

                for one_tuple in tp_and_wk:
                    get_topic_process = multiprocessing.Process(target=self.get_search_topic, args=(one.keys(), one_tuple))
    ...
    def get_topic(self, key, value, s):
        ...
        for ntopic in weibo_topic:
            print "检测到的新事件", ntopic

        print "保存到的话题最新数量", len(WeiboPage.all_all_topic)
        for stopicp in WeiboPage.all_all_topic:
            print "保存的话题有：", stopicp
...
#  这里不知道为什么报错-------------------------------------------------------------------------------
    ...
    def update(self):
        ...
        """
        # 先更新
        for uid, afore_tuple_list in WeiboPage.searched_tpdict.items():
            for afore_tuple in afore_tuple_list:
                update_process = multiprocessing.Process(target=self.get_search_topic, args=(uid, afore_tuple))
    ...
    def detection(self):
        ...
        for acc in rand_account:
            ...
            weibo_page = WeiboPage(account[acc])  # 每一个帐号去检测

            s = multiprocessing.Semaphore(2)
            detection_process = multiprocessing.Process(target=weibo_page.get_page, args=(1, 2, s))
    ...
    def run(self):
        ...
        :return:
        """
        print "再次更新长度为x的话题",len(WeiboPage.searched_tpdict)
        if WeiboPage.searched_tpdict:
            print "开始更新"
            self.update()
        else:
            print "无内容，不更新"
...
def main_weibo():
    MoblieWeibo().login('', '')
    weibopage = WeiboPage(2803301701)
    while True:
        weibopage.run()
        sleep(120)
